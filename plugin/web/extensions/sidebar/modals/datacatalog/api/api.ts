...
export type DataCatalogConfigData = { name: string; type: string; url: string; layer?: string[] };
...
type RawRawDataCatalogItem = {
  ...
  openDataUrl?: string;
  config?: { data?: { name: string; type: string; url: string; layer?: string[] | string }[] };
  order?: number;
  // bldg only fields
...
export type RawDataCatalogItem = Omit<RawRawDataCatalogItem, "layers" | "config"> & {
  layers?: string[];
  config?: { data?: DataCatalogConfigData[] };
};
...
export function modifyDataCatalog(
  ...
  return {
    ...
    ...(d.layers ? { layers: getLayers(d.layers) } : {}),
    ...(d.config
      ? {
          config: {
            ...(d.config.data
